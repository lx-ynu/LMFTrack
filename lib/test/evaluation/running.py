...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    for key, data in output.items():
        ...
        if key == 'cspg_records':
            csv_file = '{}_cspg.csv'.format(base_results_path)
            ...
            with open(csv_file, 'w', newline='') as f:
                ...
                for row in data:
                    writer.writerow({
                        'frame_id': row.get('frame_id', -1),
                        'selected_class': row.get('selected_class', None),
                        'selected_class_index': row.get('selected_class_index', -1),
                        'semantic_confidence': row.get('semantic_confidence', -1.0),
                        'template_update_flag': row.get('template_update_flag', 0),
                        'tracking_score': row.get('tracking_score', -1.0),
                    })
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    def _results_exist():
        if seq.object_ids is None:
            return os.path.isfile(os.path.join(tracker.results_dir, f'{seq.name}.txt'))
        bbox_files = [os.path.join(tracker.results_dir, f'{seq.name}_{obj_id}.txt') for obj_id in seq.object_ids]
        return all(os.path.isfile(path) for path in bbox_files)
