...
class RGBT234Dataset(BaseDataset):
    """RGBT234 benchmark loader."""

    def __init__(self):
        super().__init__()
        self.base_path = Path(self.env_settings.rgbt234_path)
        if not self.base_path.is_dir():
            raise FileNotFoundError(f'RGBT234 directory not found: {self.base_path}')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        if len(visible_frames) != len(infrared_frames):
            raise RuntimeError(f'RGB/TIR frame-count mismatch in RGBT234 sequence: {sequence_name}')
        return Sequence(sequence_name, [visible_frames, infrared_frames], 'rgbt234', ground_truth)
