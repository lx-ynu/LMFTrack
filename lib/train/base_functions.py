...
# Dataset used for the released LMFTrack training configuration.
...
def names2datasets(name_list: list, settings, image_loader):
    """Build the LasHeR train/test splits used by LMFTrack."""
    if not isinstance(name_list, list):
        raise TypeError("name_list must be a list.")

    supported = {"LasHeR_train", "LasHeR_test"}
    unknown = [name for name in name_list if name not in supported]
    if unknown:
        raise ValueError(f"Unsupported training dataset(s): {unknown}. Supported values: {sorted(supported)}")

    datasets = []
    for name in name_list:
        if name == "LasHeR_train":
            datasets.append(LasHeR(settings.env.lasher_train_dir, split='train', image_loader=image_loader))
        elif name == "LasHeR_test":
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "fusion_blocks" in n and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "fusion_blocks" not in n and p.requires_grad],
