...
def jpeg4py_loader(path):
    """Image reading using jpeg4py, with a graceful fallback when unavailable."""
    if jpeg4py is None:
        return None
    try:
...
def jpeg4py_loader_w_failsafe(path):
    """Read with jpeg4py when available, otherwise fall back to OpenCV."""
    try:
        if jpeg4py is not None:
            return jpeg4py.JPEG(path).decode()
        raise RuntimeError("jpeg4py is unavailable")
    except Exception:
        try:
            im = cv.imread(path, cv.IMREAD_COLOR)
